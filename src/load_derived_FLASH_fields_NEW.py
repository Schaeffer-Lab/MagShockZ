...
def derive_fields(path_to_data:str, rqm:int = 100) -> yt.data_objects.static_output.Dataset:
    ...
    def make_ion_2_number_density(field, data):
        ...
        ion2_number_density = ion2_percentage*data["flash","edens"]/yt.units.m # we did this because Zbar is ye/sumy, so we can just use edens instead to get the number density in units that are useful to osiris
    ...
    def make_ion_1_number_density(field, data):
        ion2_percentage = (data["flash","sumy"]-1/molar_weights[ion_1])/(1/molar_weights[ion_2]-1/molar_weights[ion_1])
        ion1_number_density = (1-ion2_percentage)*data["flash","edens"]/yt.units.code_mass

...
def derive_fields(path_to_data:str, rqm:int = 100) -> yt.data_objects.static_output.Dataset:
    ...
    def make_magnesium_number_density(field, data):
        mg_percentage = (data["flash","sumy"]-1/molar_weights["aluminum"])/(1/molar_weights['magnesium']-1/molar_weights["aluminum"])
    ...
    try:
        ...
        print(f"Error adding field mgdens: {e}")
    ...
    def make_vth_mg(field, data):
        return np.sqrt(data['flash','tion']*yt.units.kb_cgs/(yt.units.electron_mass_cgs*rqm*(molar_weights["magnesium"]/molar_weights["aluminum"])))
    ...
    try:
        ...
        print(f"Error adding field vthmg: {e}")

...
# converges to the physical value (the ~rqm_factor offset in magnetization_1D.png
# disappears); M_A, beta_e and T_e/T_i remain conserved. NOTE: real ion mass makes
# the ion gyrotime ~100x longer (tmax) and the ion scales ~10x larger, so this is a
# validation/convergence setup, not a cheap production run.

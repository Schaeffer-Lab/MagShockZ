"""scripts/warpx_make_deck.py — render the WarpX heater-piston deck from its run spec.
...
-----
    ...
    python scripts/warpx_make_deck.py --config runs/magshockz_2d_heater.warpx.yaml
        [--smoke] [--no-heater] [--output-dir ...]

    # Neither of these writes anything, so both are safe against a running job;
    # both exit 1 on drift.
    python scripts/warpx_make_deck.py --config ...yaml --check    # deck on disk vs spec
    python scripts/warpx_make_deck.py --config ...yaml --verify   # what WarpX ran vs spec
"""
...
def run_env(config: dict, deck_dir: str, deck_name: str) -> str:
    ...
    return "\n".join([
        "# Generated by scripts/warpx_make_deck.py. Sourced by init_warpx/run_heater_2d.sbatch.",

...
def set_uth_si( STATE ):
    ...
    for start in range(0, len(STATE["u"][:,0]), chunk_size):
        ...
        STATE["u"][start:end, 0] = STATE['vthsi']((STATE["x"][start:end, 1], STATE["x"][start:end, 0]))
        STATE["u"][start:end, 1] = STATE['vthsi']((STATE["x"][start:end, 1], STATE["x"][start:end, 0]))
        STATE["u"][start:end, 2] = STATE['vthsi']((STATE["x"][start:end, 1], STATE["x"][start:end, 0]))
...
def load_and_interpolate_density(STATE, filename):
    ...
    STATE["xmin"] = np.array([-3362.0, 1801.0])
